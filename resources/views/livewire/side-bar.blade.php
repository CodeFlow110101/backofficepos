...
<div class="py-12 px-6 h-full flex flex-col justify-content-between">
    <div class="h-min grid grid-cols-1 gap-4">
        ...
        <a href="/stock" wire:navigate class="flex justify-start gap-2 items-center font-medium p-2 @if(in_array($path ,['stock' , 'manage-stock'])) bg-amber-500 @else text-black/70 hover:bg-amber-500 transition-colors duration-200 @endif rounded-lg group">
            <div>
                <svg class="w-5 h-5 @if(in_array($path ,['stock' , 'manage-stock'])) text-white @else text-black/70 group-hover:text-white transition-colors duration-200 @endif" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.143 4H4.857A.857.857 0 0 0 4 4.857v4.286c0 .473.384.857.857.857h4.286A.857.857 0 0 0 10 9.143V4.857A.857.857 0 0 0 9.143 4Zm10 0h-4.286a.857.857 0 0 0-.857.857v4.286c0 .473.384.857.857.857h4.286A.857.857 0 0 0 20 9.143V4.857A.857.857 0 0 0 19.143 4Zm-10 10H4.857a.857.857 0 0 0-.857.857v4.286c0 .473.384.857.857.857h4.286a.857.857 0 0 0 .857-.857v-4.286A.857.857 0 0 0 9.143 14Zm10 0h-4.286a.857.857 0 0 0-.857.857v4.286c0 .473.384.857.857.857h4.286a.857.857 0 0 0 .857-.857v-4.286a.857.857 0 0 0-.857-.857Z" />
                </svg>
            </div>
            <div class="@if(in_array($path ,['stock' , 'manage-stock'])) text-white @else group-hover:text-white transition-colors duration-200 @endif">Stock</div>
        </a>
        <a href="/delivery" wire:navigate class="flex justify-start gap-2 items-center font-medium p-2 @if(in_array($path ,['delivery' , 'manage-delivery'])) bg-amber-500 @else text-black/70 hover:bg-amber-500 transition-colors duration-200 @endif rounded-lg group">
            <div>
                <svg class="w-5 h-5 @if(in_array($path ,['delivery' , 'manage-delivery'])) text-white @else text-black/70 group-hover:text-white transition-colors duration-200 @endif" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <a href="/user" wire:navigate class="flex justify-start gap-2 items-center font-medium p-2 @if(in_array($path ,['user' , 'manage-user'])) bg-amber-500 @else text-black/70 hover:bg-amber-500 transition-colors duration-200 @endif rounded-lg group">
            ...
            <div class="@if(in_array($path ,['user' , 'manage-user'])) text-white @else group-hover:text-white transition-colors duration-200 @endif">Users</div>
        </a>
    </div>
