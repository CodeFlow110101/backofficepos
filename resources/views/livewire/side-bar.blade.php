...
<div class="py-12 px-6 h-full flex flex-col justify-content-between">
    <div class="h-min grid grid-cols-1 gap-4">
        <a href="/warehouse" wire:navigate class="flex justify-start gap-2 items-center font-medium p-2 @if(in_array($path , ['warehouse' , 'add-edit-warehouse'])) bg-amber-500 @else text-black/70 hover:bg-amber-500 transition-colors duration-200 @endif rounded-lg group">
            <div>
                <svg class="w-5 h-5 @if(in_array($path , ['warehouse' , 'add-edit-warehouse'])) text-white @else text-black/70 group-hover:text-white transition-colors duration-200 @endif" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m4 12 8-8 8 8M6 10.5V19a1 1 0 0 0 1 1h3v-3a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v3h3a1 1 0 0 0 1-1v-8.5" />
                </svg>
            </div>
            <div class="@if(in_array($path ,['warehouse' , 'add-edit-warehouse'])) text-white @else group-hover:text-white transition-colors duration-200 @endif">Warehouse</div>
        </a>
