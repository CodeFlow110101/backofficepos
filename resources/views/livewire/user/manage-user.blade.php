...
state(['name', 'email', 'role', 'password', 'password_confirmation', 'id']);
            ...
            <div class="w-1/2">
                <div class="flex justify-center size-full items-center border border-black/30 rounded-xl">
                    <form wire:submit="submit" class="h-full flex flex-col gap-8 py-8 px-4 w-11/12">
                        ...
                        <div>
                            <input wire:model="email" type="text" class="border w-full rounded-lg text-black/70 border-black/30 outline-none p-3" placeholder="Email">
                            @error('email')
                            <div wire:transition.in.scale.origin.top.duration.1000ms class="text-red-500 text-sm">
                                <span class="error">{{ $message }}</span>
                            </div>
                        ...
                        <div>
                            <select wire:model="role" class="border w-full rounded-lg text-black/70 border-black/30 capitalize outline-none p-3">
                        ...
                        <div>
                            <input wire:model="password" type="password" class="border w-full rounded-lg text-black/70 border-black/30 outline-none p-3" placeholder="password">
                            @error('password')
                            <div wire:transition.in.scale.origin.top.duration.1000ms class="text-red-500 text-sm">
                                <span class="error">{{ $message }}</span>
                            </div>
                        ...
                        <div>
                            <input wire:model="password_confirmation" type="password" class="border w-full rounded-lg text-black/70 border-black/30 outline-none p-3" placeholder="Retype Password">
